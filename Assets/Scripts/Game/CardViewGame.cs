...
public class CardViewGame : MonoBehaviour
{
	...
	public void Init(Card card, Player owner)
	{
		...
		rarityBackgroundImage.color = CardColorUtil.GetRarityColor(card.Points);
		pointsBackgroundImage.color = CardColorUtil.GetRarityColor(card.Points);
		suitBackgroundImage.color = CardColorUtil.GetSuitColor(card.Suit);
